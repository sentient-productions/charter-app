...
def _create_table_prompt(df: pd.DataFrame, title: str):
    ...
    for header in df.columns:
        column_type = 'text'
        try:
            column_type_str = df[header].dtype.name.lower()
            if column_type_str == 'int64' or column_type_str == 'int32':
                column_type = 'int'
            elif column_type_str == 'float64':
                column_type = 'real'
            elif column_type_str == 'datetime64':
...
class PromptBuilder(object):
    def __init__(self, args):
        self.args: BinderParameters = args
