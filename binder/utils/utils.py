...
def majority_vote(
    ...
    if vote_method == "simple":
        sorted_candi_answer_list = sorted(
            list(candi_answer_dict.items()),
            key=cmp_to_key(_compare_answer_vote_simple),
            reverse=True,
        )
    elif vote_method == "prob":
        sorted_candi_answer_list = sorted(
            list(candi_answer_dict.items()),
            key=cmp_to_key(_compare_answer_vote_with_prob),
            reverse=True,
        )
    ...
    pred_answer, pred_answer_nsqls = (
        list(pred_answer_info[0]),
        pred_answer_info[1]["nsqls"],
    )
...
def load_data_split(
    dataset_to_load,
    split="validation",
    data_dir=os.path.join(ROOT_DIR, "binder_datasets/"),
):

    if (
        dataset_to_load.split(".")[-1] == "csv"
        or dataset_to_load.split(".")[-1] == "tsv"
    ):
        sep = "\t" if dataset_to_load.split(".")[-1] == "tsv" else ","
        nrows = 3 if split == "header" else None
        return [
            pd.read_csv(
                os.path.join(data_dir, dataset_to_load), sep=sep, nrows=nrows
            ).convert_dtypes()
        ]

    config_name = (
        "Outdoors_v1_00" if dataset_to_load == "amazon_us_reviews" else None
    ...
    if dataset_to_load in [
        ...
        for data_item in dataset_split_loaded:
            ...
            if split == "test" and data_item['table']["id"] not in small_test_ids_for_iter:
                continue
        ...
        for data_item in dataset_split_loaded:
            ...
            new_dataset_split_loaded.append(data_item)
        dataset_split_loaded = new_dataset_split_loaded
    elif dataset_to_load in ["amazon_us_reviews"]:
        dataset_split_loaded = [
            dataset_split_loaded.data.table
        ]  # todo implement parametrization of category
    else:
