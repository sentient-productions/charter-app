...
import {Save as SaveIcon, ShowChart as ShowChartIcon, TableRows as TableRowsIcon} from '@mui/icons-material';
...
export default function QueryBar({state, setState}) {
    ...
    const handleQuerySubmit = () => {
        ...
        try {
          ...
          formData.append("query", query == "" ? DATASET_QUERIES[state.queryMode][state.dataset] : query);
          ...
          .then(jsonText => {
            if (state.queryMode == QUERY_MODES.PLOT) {
                try {
                    ...
                    setState({...state, errorText: "An error occurred while parsing the response. Please modify your query and try again."})
                    setIsLoading(false)
                }
            } else {
                try {
                    ...
                    <InputAdornment sx={{mr: 2}}> 
                        ...
                        {(popupState) => (
                            ...
                            <Menu {...bindMenu(popupState)}>
                                <MenuItem onClick={() => {
                                    setState({...state, responseCode: "", isExecuted: false, queryMode: QUERY_MODES.PLOT, defaultQuery: DATASET_QUERIES[state.queryMode][state.dataset], errorText: ""})
                                    Plotly.purge('graph-div');
                                    popupState.close()
                                }}>
                                <ShowChartIcon /> 
                                ...
                                <MenuItem onClick={() => {
                                    setState({...state, responseCode: "", isExecuted: false, queryMode: QUERY_MODES.TABLE, defaultQuery: DATASET_QUERIES[state.queryMode][state.dataset], errorText: ""})
                                    Plotly.purge('graph-div');
                                    popupState.close()
                                }}>
                                <TableRowsIcon /> 
