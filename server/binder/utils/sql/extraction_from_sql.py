  ...
  """
    ...
    def choose_best_table(default_tables, tok):
        ...
        candidate = process.extractOne(
            lower_tok, [table.lower() for table in default_tables]
        )[0]
        return candidate
...
def parse_col_unit(
    toks, start_idx, tables_with_alias, schema, default_tables=None, end_idx=None
):
    """
...
def parse_sql(toks, start_idx, schema):
    ...
    from_end_idx, default_tables, tables_with_alias = parse_from(
        toks, start_idx, schema
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, choices=["template_extraction", "schema_extraction"]
    )
    ...
    if args.task == "schema_extraction":
        if args.mode == "debug":
            ...
            sql = (
                sql
                + " EXCEPT "
                + 'SELECT T1.year, T1.sacks FROM players AS T1 JOIN tackles AS T2 ON T1.id = T2.player_id WHERE T2.manager = "A" and T2.season NOT IN (SELECT season FROM match WHERE match_name = "IVL" INTERSECT SELECT T1.year, T1.sacks FROM sack AS T1) GROUP BY T1.year, T1.sacks HAVING count(T1.coach) > 10 ORDER BY T2.score LIMIT 5'
            )
