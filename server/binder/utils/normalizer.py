...
from server.binder.utils.sql.all_keywords import ALL_KEY_WORDS
from server.binder.utils.sql.extraction_from_sql import *
...
def prepare_df_for_neuraldb_from_table(
    table: Union[Dict, datasets.table.MemoryMappedTable],
    add_row_id=True,
    normalize=True,
    lower_case=True,
) -> pd.DataFrame:
    if isinstance(table, (datasets.table.MemoryMappedTable, pyarrow.Table)):
            ...
            if (header in sql_str) and (header not in ALL_KEY_WORDS):
                ...
                for matched_of_this_header in all_matched_of_this_header:
                    ...
                    if (
                        ...
                        and (not sql_str[start_idx - 1] == "`")
                        and (not sql_str[end_idx] == "`")
                    ):
                        have_matched[start_idx:end_idx] = [
